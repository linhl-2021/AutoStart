...
def login_web(url: str,ntos_page,firewalldWebuser,firewalldWebpasswd):
    publickey =  getpublickey(ntos_page)
    username=rsa_data(firewalldWebuser,publickey)
    #print(username)
    password=rsa_data(firewalldWebpasswd,publickey)
    body = {
            "username":username,
            "password":password,
    ...
    logger.debug(f"登录缓存： {login_dict}") 
    return login_dict
